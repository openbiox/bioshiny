#' Function to run bioshiny shiny APP service
#'
#' @param appDir The application to run.
#' Default is system.file('extdata', 'shiny/R', package = 'bioshiny')
...
#' }
web <- function(appDir = system.file("extdata", "shiny", package = "bioshiny"), auto_create = FALSE, 
  ...) {
  params <- list(...)
...
#' @param shiny_config_file bioshiny shiny configuration file
...
set_shiny_workers <- function(n, shiny_config_file = Sys.getenv("BIOSHINY_CONFIG", 
  system.file("extdata", "config/shiny.config.yaml", package = "bioshiny")), auto_create = FALSE) {
  ...
  for (i in 1:n) {
    system(sprintf("Rscript %s &> %s/worker_%s.log &", worker_script, log_dir, 
      stri_rand_strings(1, 20)))
  }
}

#' Function to copy the default plugins of bioshiny
#' 
#' @param plugin_dir The destdir to store plugins [~/.bioshiny/plugins]
...
copy_plugins <- function(plugin_dir = "~/.bioshiny/plugins", template_dir = system.file("extdata", 
  "config/", package = "bioshiny"), pattern = "shiny.*.parameters.toml", auto_create = FALSE) {
...
#' Function to copy the default configuration file of bioshiny
#' 
#' @param config_dir The destdir to store plugins [~/.bioshiny]
#' @param template_dir The template dir system.file('extdata', 'config/shiny/', 
#' package = 'bioshiny')
